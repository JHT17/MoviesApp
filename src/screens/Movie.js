...
export default function Movie(props) {
  ...
  const {language}= useLanguage();
    ...
    useEffect(() => {
        getMovieByIdApi(id,language).then((response)=>{
           setMovie(response);
       });
    
    }, [language]);

    // condicion de espera de contenido a renderizar 
...
function MovieImage(props){
    ...
    return(
        <View style={styles.viewPoster}>
            <Image style={styles.poster} source={{ uri: `${BASE_PATH_IMG}/w185${posterPath}`}} />
        </View>
    );
}
    ...
    return(
        <View style={styles.viewRating}>
            <Rating  
                ...
                rattingColor= "#ffc205"
                rattingBackgroundColor= {theme === "dark" ? "#192734" : "#f0f0f0"}
                startingValue={media}
                imageSize={20}
